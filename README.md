...
- следуя [инструкциям в официальной документации Ethereum](http://www.ethdocs.org/en/latest/mining.html#using-geth) включить майнинг в тестовой сети примерно на 20 - 30 минут.
- подключиться к каналу команды Airalab в Gitter: [Aira team friends](https://gitter.im/airalab/friends) и написать свой адрес в тестовой сети. Кто нибудь обязательно вам поможет!  
...
- Обратиться к `DAO factory` и найти контракт сборщика с названием `Emission regulator via Market trade`, при обращении к данному сборщику необходимо указать `адрес токена внутренней валюты ДАО`, `адрес, на который будет происходить эмиссия новый токенов`, а также `процент эмиссируемых новых токенов`.
- Обратиться к `DAO factory` и найти контракт сборщика с названием `Emission regulator via Market trade`,
