...
Отличие данных способов перечисления токенов акций заключается в формате согласия. В первом случае получатель `recipient` не даёт формального согласия на вход в соучередители. Во втором же мы протоколируем согласие функцией `sign()` и только после этого переводим токены акций.
