...
```js
marketRegulator.sign({from: eth.accounts[0], gas: 900000});
```
...
var lesson = eth.contract(Lesson_8).at(learning_center.getModule("Lesson_8"));
...
var tokenair = eth.contract(tokenair_abi).at(learning_center.getModule("Air token"));
tokenair.getBalance()
```
