...
Create the `Shareholder` contract. In order to do that call the `Aira BuilderShareholder` -> `Create`. Provide address of the shares, amount, address of the recipient.
...
Now we have to grant token controlling ability to `Shareholder` contract. Add the `Shares` contract to the contract list:
...
We are ready to sign shareholder contract. Let's call the `Sign` function by sending the transaction from the contract creator address. 
