...
Create the `ShareSale` contract using builder
...
Send shares to the `ShareSale` contract
...
Check that the shares are received

```js
shares.balanceOf(web3.eth.accounts[0])
```

Execute lesson. Don't forget to pass the `shareSale` address
